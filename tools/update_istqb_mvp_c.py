...
QPDF_C = Path('/mnt/data/ISTQB_FL_v4.0_샘플문제_C_v1.6_한글_v1.0.pdf')
APDF_C = Path('/mnt/data/ISTQB_FL_v4.0_샘플문제_C_v1.6_정답과_해설_한글_v1.0.pdf')
A_JSON = OUT / 'data' / 'istqb-v1.7a.json'
B_JSON = OUT / 'data' / 'istqb-v1.7b.json'
...
def join_ko_lines(lines):
    ...
    for line in lines:
        ...
        starts_new = bool(re.match(
            r'^(?:•|[-]|[ivxⅰ-ⅴ]+\.|[A-Z]{1,4}\d{0,3}|TC\s?\d+|P\d+|R\d+|\d+\.|\(\d+\)|규칙|조건|결과|위험 수준|프로젝트|테스트 우선순위|번호|테스트|환경 구성|시작|종료|총 테스트|Given:|When:|Then:|And:|A\.|B\.|C\.|D\.|INPUT:|IF|ELSE|E\(n\)|A\(n\))',
            line
        ))
        if starts_new or prev.endswith((':', '?', '다:', '같다.', '같다:', '이다:', '했다:', '한다:', '있다:', '이다.')):
            out.append(line)
        else:
            if re.match(r'^(•|[ivxⅰ-ⅴ]+\.|a\)|b\)|c\)|d\)|e\))', prev):
                out[-1] = prev + ' ' + line
            else:
                out[-1] = prev + ' ' + line
    return out
...
<body>
  <main class="app" id="app">
    ...
    <section class="screen" id="selectScreen">
      ...
      <div class="card">
        ...
        <p class="tiny" id="datasetHint">현재 MVP 데이터: V1.7A 66문항 + V1.7B 40문항 + V1.6 40문항. 표/그림이 필요한 일부 문항은 원본 페이지 이미지를 함께 표시합니다.</p>
      </div>
    </section>
  ...
  <footer class="footer">Source: ISTQB® / KSTQB Sample Exam A, B, C. 학습용 MVP.</footer>
...
const WRONG_KEY = "istqb_wrong_ids_v3";
const BOOKMARK_KEY = "istqb_bookmarks_v3";
...
function startQuiz(questions) {
  if (!questions.length) {
    alert("선택한 조건에 해당하는 문제가 없습니다. 현재 추가문제는 V1.7A에만 포함되어 있습니다.");
    return;
  }
...
def write_reports(c_diag, all_questions):
    rows = []
    ...
    warnings = []
    if c_diag['questionCount'] != 40:
        warnings.append(f'V1.6 문항 수가 40이 아닙니다: {c_diag["questionCount"]}')
    if c_diag['answerCount'] != 40:
        warnings.append(f'V1.6 정답표 매칭 수 확인 필요: {c_diag["answerCount"]}')
    if c_diag['explanationCount'] != 40:
        warnings.append(f'V1.6 해설 매칭 수 확인 필요: {c_diag["explanationCount"]}')
    for qid in c_diag['badChoiceCounts']:
        warnings.append(f'V1.6 {qid}: 보기 수 확인 필요')
    for qid in c_diag['missingAnswers']:
        warnings.append(f'V1.6 {qid}: 정답 누락')
    for qid in c_diag['missingExplanations']:
        warnings.append(f'V1.6 {qid}: 해설 누락')
    ...
    md.append('')
    md.append('## V1.6 신규 파싱 결과')
    md.append(f'- 문항 수: {c_diag["questionCount"]}')
    md.append(f'- 정답표 매칭: {c_diag["answerCount"]} / 40')
    md.append(f'- 해설 매칭: {c_diag["explanationCount"]} / 40')
    md.append(f'- 원본 표/그림 보조 이미지 포함 문항: {", ".join(c_diag["visualQuestionIds"])}')
    md.append(f'- 복수 정답 문항: {", ".join(c_diag["multiAnswer"]) if c_diag["multiAnswer"] else "없음"}')
    md.append('')
    ...
    if warnings:
        ...
        md.append('- V1.6 자동 검증에서 큰 이상 없음')
    ...
    c_md = []
    c_md.append('# ISTQB Foundation V1.6 PDF 파싱 검증 리포트')
    c_md.append('')
    c_md.append(f'- 총 문항 수: {c_diag["questionCount"]}')
    c_md.append(f'- 정답표 매칭: {c_diag["answerCount"]} / 40')
    c_md.append(f'- 해설 매칭: {c_diag["explanationCount"]} / 40')
    c_md.append(f'- 보기 수 분포: {c_diag["choiceCounts"]}')
    c_md.append(f'- 원본 표/그림 보조 이미지 포함 문항: {", ".join(c_diag["visualQuestionIds"])}')
    c_md.append(f'- 복수 정답 문항: {", ".join(c_diag["multiAnswer"]) if c_diag["multiAnswer"] else "없음"}')
    c_md.append('')
    c_md.append('## 수동 확인 권장')
    if warnings:
        for w in warnings:
            c_md.append(f'- {w}')
    else:
        c_md.append('- 자동 검증에서 큰 이상 없음')
    (OUT / 'parse_report_v1.6.md').write_text('\n'.join(c_md), encoding='utf-8')
...
if __name__ == '__main__':
    ...
    expected_c = [str(i) for i in range(1, 41)]
    bundle_c, diag_c = build_bundle(
        question_pdf=QPDF_C,
        answer_pdf=APDF_C,
        version='V1.6',
        version_slug='v1.6',
        set_name='C',
        expected_qids=expected_c,
        answer_summary_pages=[2],
        explanation_page_indices=range(3, 38),
        visual_page_map={'21': 11, '22': 11, '23': 12, '24': 13, '31': 16, '32': 17},
    )
    (OUT / 'data' / 'istqb-v1.6.json').write_text(json.dumps(bundle_c, ensure_ascii=False, indent=2), encoding='utf-8')

    a_data = json.loads(A_JSON.read_text(encoding='utf-8'))
    b_data = json.loads(B_JSON.read_text(encoding='utf-8'))
    all_questions = a_data['questions'] + b_data['questions'] + bundle_c['questions']
    all_bundle = {
        'meta': {
            'title': 'ISTQB FOUNDATION 문제은행',
            'version': 'ALL',
            ...
            'includedVersions': ['V1.7A', 'V1.7B', 'V1.6'],
        },
    ...
    write_app_files()
    write_reports(diag_c, all_questions)
    write_readme()
    shutil.copy('/mnt/data/update_istqb_mvp_c.py', OUT / 'tools' / 'update_istqb_mvp_c.py')
    zip_path = shutil.make_archive('/mnt/data/istqb_foundation_mvp', 'zip', OUT)
    print('Wrote', zip_path)
    print('C diagnostics:', diag_c)
